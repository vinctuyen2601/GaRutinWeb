...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    description: product.seoDescription || product.description || `Mua ${product.name} tại GaRutin. Giá ${formattedPrice}/${product.unit}.`,
    openGraph: {
      title: product.name,
          ...
          <div className="space-y-5">
            <div>
              ...
              {product.weightPerUnit && (
                <p className="text-sm text-gray-500">Trọng lượng: {product.weightPerUnit}/{product.unit}</p>
              )}
              <div className="flex items-center gap-3 mt-3">
                <span className="text-3xl font-bold text-primary-600">{formatVND(price)}</span>
                {product.salePrice && (
              ...
              {product.stockStatus === 'out_of_stock' && (
                <p className="text-red-500 text-sm mt-1 font-medium">⚠️ Tạm hết hàng</p>
              )}
              {product.stockStatus === 'pre_order' && (
                <p className="text-blue-500 text-sm mt-1 font-medium">📋 Nhận đặt trước</p>
              )}
            </div>
